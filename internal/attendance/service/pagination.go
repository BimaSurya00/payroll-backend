...
func NewPagination(data []*dto.AttendanceResponse, page, perPage int, total int64, path string) *Pagination[*dto.AttendanceResponse] {
	...
	// Convert slice
	dataSlice := make([]*dto.AttendanceResponse, len(data))
	for i, v := range data {
		dataSlice[i] = v
	}

	return &Pagination[*dto.AttendanceResponse]{
		CurrentPage:  page,
		Data:         dataSlice,
