...
func main() {
	...
	applied := getAppliedMigrations(ctx, conn)
...
func ensureMigrationsTable(ctx context.Context, conn *pgx.Conn) {
	_, err := conn.Exec(ctx, `
...
func getAppliedMigrations(ctx context.Context, conn *pgx.Conn) map[string]bool {
